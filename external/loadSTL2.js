...
var parseStlBinary = function(stl, color) {
    ...
    var dd = Math.max(dx, Math.max(dy, dz));
	...
	for (var i = 0; i < triangles; i++) {
		...
		// Get all 3 vertices for this triangle
		for (var j = 0; j < 3; j++) {
            var currentvector = new THREE.Vector3( dv.getFloat32(offset, isLittleEndian), dv.getFloat32(offset+4, isLittleEndian), dv.getFloat32(offset+8, isLittleEndian) );
